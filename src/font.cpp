...
void Font_Static::create(void const* ttf_data, size_t _size, float _height, VkDescriptorSet set, VkSampler sampler) {
	...
	error = FT_New_Memory_Face(engine.font.library, (FT_Byte const*)ttf_data, (FT_Long)_size, 0, &face);
...
void Font_Static::destroy() {
    if (atlas_view)       vkDestroyImageView(engine.graphics.device, atlas_view, nullptr);
	if (atlas_allocation) vmaDestroyImage(engine.graphics.allocator, atlas_image, atlas_allocation);
}
