...
string find_command_arguments(string s) {
	...
	if (!found_space) return string { s.data + s.count, 0 };
	...
	return string { s.data + cursor, s.count - cursor };
}
...
void Console_Layer::handle_character_input(c32 ch) {
	switch (ch)
	{
	// ignored
	case ESCAPE:
		...
		if (hglb != NULL)
		{
			...
			if (lptstr != NULL)
			{
				while (input.count < 71 && *lptstr != 0) {
					char nch = *lptstr++;
					if(acceptable_character(nch))
						input.data[input.count++] = nch;
				}
...
#undef debug_print
	...
	break; case '\r': {
		...
		// Execute Command
			...
			console::print("\x1b\xFF\x25\x25unknown command: ");
	...
	for (auto it = events.begin(); it != events.end(); ) {
		...
		case Event_Key_Down: {
			...
			if(flags & layer::show) {
				...
				break; case keys::Up: {
					current_command = math::min(current_command+1, (s64)command_history_ends.size()-1);
				}
				break; case keys::Down: {
					current_command = math::max(current_command-1, -1);
				}
				break; case keys::Left: {
					input_cursor = math::max(input_cursor-1, 2);
				}
				break; case keys::Right: {
					input_cursor = math::min(input_cursor+1, input.count);
				}
				break; case keys::Mouse_WheelUp: {
					++buffer_view_offset;
				}
				break; case keys::Mouse_WheelDown: {
					buffer_view_offset = math::max(buffer_view_offset-1, 0);
				}
...
#               if defined(FISSION_PLATFORM_MACOS) // Hotfix, TODO
				break; case keys::Enter: handle_character_input('\r');
				break; case keys::Back:  handle_character_input('\b');
#               endif
				}
				it = events.erase(it);
				continue;
		...
		case Event_Character_Input: {
			if (flags & layer::show) {
				handle_character_input(it->character_input.codepoint);
				it = events.erase(it);
				continue;
			}
			break;
...
string get_string(c8 const* cursor, c8 const* start) {
	...
	if (cursor == end) return string { cursor, 0 };
	
	return string { cursor, u64(end - cursor) };
}
...
void add_string(Textured_Renderer_2D& r, Color_Info& info, string str, v2f32 pos) {
	...
	for_n (str.count) {
		...
		if (c == ESCAPE) {
			...
			glyph = engine.font.console.lookup(' ');
			pos.x += glyph->advance * 4;
			continue;
		}

		glyph = engine.font.console.lookup(c);
...
void Console_Layer::draw_console_buffer(Textured_Renderer_2D& r, float top, float ystride) {
...

	{
		...
		buffer_view_offset = math::min(buffer_view_offset, newline_count);
...
void Console_Layer::on_update(double dt, Render_Context* ctx) {
	...
	r2d.add_rect({ 0, screen_width, position, position + 1 }, colors::gray_web);
	...
	// Show from history
		...
		string_size = tr2d.add_string("> ", {4, position}, colors::white);
