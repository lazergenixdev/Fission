...
def compile(shader: str, ext: str):
	output(COLOR_RED)
	r = os.system(f"{shader_compiler} {shader_options} {shader}.{ext}")

	if r != 0:
		panic(f"error: shader compilation failed with code {r}")
	else:
		output(COLOR_RESET)
		print(f"compiled shader: \"{shader}.{ext}\"")
	
	r = os.system(f".\\file_to_cpp.exe a.spv {shader}.{ext}.inl")

	if r != 0:
		panic(f"error: file_to_cpp failed to generate .inl file [exit_code={r}]")
	else:
		print(f"generated shader include: \"{shader}.{ext}.inl\"")
